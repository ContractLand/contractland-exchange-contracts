...
contract.only('OrderHistory',  async(accounts) => {
  ...
  describe("Add", async() => {
    ...
    it("should append new order to end of list", async() => {
      ...
      await orderHistoryTest.add(order1.id, order1.price, order1.originalAmount, order1.amount, order1.isSell, order1.timestamp).should.be.fulfilled
      await orderHistoryTest.add(order2.id, order2.price, order2.originalAmount, order2.amount, order2.isSell, order2.timestamp).should.be.fulfilled
      await orderHistoryTest.add(order3.id, order3.price, order3.originalAmount, order3.amount, order3.isSell, order3.timestamp).should.be.fulfilled
  ...
  describe("GetOrders", async() => {
    it("should not exceed limit", async() => {
      const order = {price: 1, originalAmount: 1, amount: 1, isSell: false, timestamp: 1}

      await orderHistoryTest.add(1, order.price, order.originalAmount, order.amount, order.isSell, order.timestamp).should.be.fulfilled
      await orderHistoryTest.add(2, order.price, order.originalAmount, order.amount, order.isSell, order.timestamp).should.be.fulfilled
      await orderHistoryTest.add(3, order.price, order.originalAmount, order.amount, order.isSell, order.timestamp).should.be.fulfilled

      const expectedOrders = [
        {id: 3, price: 1, originalAmount: 1, amount: 1, isSell: false, timestamp: 1},
        {id: 2, price: 1, originalAmount: 1, amount: 1, isSell: false, timestamp: 1}
      ]
    ...
    it("should return all orders up to endTime when startTime == 0", async() => {
      const order1 = {id: 1, price: 1, originalAmount: 1, amount: 1, isSell: false, timestamp: 1}
      const order2 = {id: 2, price: 2, originalAmount: 2, amount: 2, isSell: true, timestamp: 2}
      const order3 = {id: 3, price: 3, originalAmount: 3, amount: 3, isSell: false, timestamp: 3}
    ...
    it("should return all orders within time range when endTime and startTime are found", async() => {
      ...
      const order2 = {id: 2, price: 2, originalAmount: 2, amount: 2, isSell: true, timestamp: 2}
      const order3 = {id: 3, price: 3, originalAmount: 3, amount: 3, isSell: false, timestamp: 3}
      const order4 = {id: 4, price: 4, originalAmount: 4, amount: 4, isSell: true, timestamp: 4}
      const order5 = {id: 5, price: 5, originalAmount: 5, amount: 5, isSell: true, timestamp: 5}

      await orderHistoryTest.add(order1.id, order1.price, order1.originalAmount, order1.amount, order1.isSell, order1.timestamp).should.be.fulfilled
      await orderHistoryTest.add(order2.id, order2.price, order2.originalAmount, order2.amount, order2.isSell, order2.timestamp).should.be.fulfilled
      await orderHistoryTest.add(order3.id, order3.price, order3.originalAmount, order3.amount, order3.isSell, order3.timestamp).should.be.fulfilled
      await orderHistoryTest.add(order4.id, order4.price, order4.originalAmount, order4.amount, order4.isSell, order4.timestamp).should.be.fulfilled
      await orderHistoryTest.add(order5.id, order5.price, order5.originalAmount, order5.amount, order5.isSell, order5.timestamp).should.be.fulfilled
    ...
    it("should return all orders within time range when endTime and startTime are not found", async() => {
      ...
      const order3 = {id: 3, price: 3, originalAmount: 3, amount: 3, isSell: false, timestamp: 3}
      const order4 = {id: 4, price: 4, originalAmount: 4, amount: 4, isSell: true, timestamp: 4}
      const order6 = {id: 6, price: 6, originalAmount: 6, amount: 6, isSell: true, timestamp: 6}

      await orderHistoryTest.add(order1.id, order1.price, order1.originalAmount, order1.amount, order1.isSell, order1.timestamp).should.be.fulfilled
      await orderHistoryTest.add(order3.id, order3.price, order3.originalAmount, order3.amount, order3.isSell, order3.timestamp).should.be.fulfilled
      await orderHistoryTest.add(order4.id, order4.price, order4.originalAmount, order4.amount, order4.isSell, order4.timestamp).should.be.fulfilled
      await orderHistoryTest.add(order6.id, order6.price, order6.originalAmount, order6.amount, order6.isSell, order6.timestamp).should.be.fulfilled
