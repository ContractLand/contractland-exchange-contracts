...
contract TestOrderHistory {
  ...
  function add(uint64 id, uint price, uint originalAmount, uint amount, bool isSell, uint64 timestamp) public {
    data.add(OrderHistory.Order(id, price, originalAmount, amount, isSell), timestamp);
  }

  function getOrders(uint64[] timeRange, uint16 limit) public view returns (uint64[], uint[], uint[], uint[], bool[], uint64[]) {
